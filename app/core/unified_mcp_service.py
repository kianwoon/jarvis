...
class MCPSubprocessPool:
    ...
    async def _ensure_initialized(self):
        """Ensure the pool is initialized with cleanup task when event loop is available"""
    ...
    def _start_cleanup_task(self):
        ...
        if self._cleanup_task is None or self._cleanup_task.done():
            try:
                ...
                logger.debug("MCPSubprocessPool: No event loop available for cleanup task yet")
    ...
    async def _check_pool_health(self):
        ...
        # Ensure pool is initialized before checking health
    ...
    async def cleanup_all(self):
        ...
        async with self._lock:
            # No need to ensure initialization for cleanup
...
# Global subprocess pool instance (lazy initialization)
_mcp_subprocess_pool = None

def get_mcp_subprocess_pool() -> MCPSubprocessPool:
    """Get the global MCP subprocess pool instance (lazy initialization)"""
    global _mcp_subprocess_pool
    if _mcp_subprocess_pool is None:
        _mcp_subprocess_pool = MCPSubprocessPool()
    return _mcp_subprocess_pool
...
class UnifiedMCPService:
    ...
    async def _get_stdio_client(self, server_config: Dict[str, Any]) -> Any:
        ...
        if command == "docker" and args:
            ...
            if len(args) >= 4 and args[0] == "exec":
                return await get_mcp_subprocess_pool().get_or_create_process(server_config)
    ...
    async def call_stdio_tool(self, server_config: Dict[str, Any], tool_name: str, 
        ...
        try:
            ...
            await get_mcp_subprocess_pool()._check_pool_health()
            ...
            try:
                pool = get_mcp_subprocess_pool()
                process_key = pool._generate_process_key(server_config)
                logger.info(f"[STDIO] Immediately cleaning up subprocess {process_key} after tool completion")
                await pool._remove_process(process_key)
...
async def cleanup_mcp_subprocesses():
    ...
    try:
        ...
        await get_mcp_subprocess_pool().cleanup_all()
...
def get_mcp_pool_stats() -> Dict[str, Any]:
    ...
    """
    try:
        pool_stats = get_mcp_subprocess_pool().get_pool_stats()
