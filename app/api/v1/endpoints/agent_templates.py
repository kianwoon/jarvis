...
# Helper endpoint to create pipeline from template
@router.post("/pipeline-templates/{template_name}/instantiate")
def instantiate_pipeline_from_template(
    ...
    # Get the template
    template_query = text("SELECT * FROM pipeline_templates WHERE name = :name AND is_active = true")
    template = db.execute(template_query, {"name": template_name}).fetchone()
    if not template:
        raise HTTPException(status_code=404, detail="Pipeline template not found or inactive")
    
    # Create the pipeline
    pipeline_query = text("""
        INSERT INTO agentic_pipelines (name, description, collaboration_mode, is_active)
        VALUES (:name, :description, :mode, true)
        RETURNING id
    """)
    try:
        result = db.execute(pipeline_query, {
            "name": pipeline_name,
            "description": description or template.description,
            "mode": template.default_config.get("mode", "sequential")
        })
        pipeline_id = result.fetchone()[0]
        
        # Add agents to pipeline
        for idx, agent in enumerate(template.agent_sequence):
            agent_query = text("""
                INSERT INTO pipeline_agents 
                (pipeline_id, agent_name, execution_order, config)
                VALUES (:pipeline_id, :agent_name, :order, :config)
            """)
            db.execute(agent_query, {
                "pipeline_id": pipeline_id,
                "agent_name": agent.get("agent", agent.get("name")),
                "order": idx,
                "config": json.dumps({
                    "role": agent.get("role"),
        ...
        return {
            "pipeline_id": pipeline_id,
            "message": f"Pipeline '{pipeline_name}' created from template '{template_name}'"
        }
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=400, detail=str(e))