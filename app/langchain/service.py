...
GENERAL ENTITY DISAMBIGUATION AND RELEVANCE FILTERING:
When processing search results or any information sources:

1. **PRIMARY SUBJECT IDENTIFICATION**:
   - Identify the MAIN ENTITY or TOPIC from the user's query
   - This is your PRIMARY FOCUS for information extraction
   - Example: If user asks about "Product X", that is the primary subject
   
2. **RELEVANCE VERIFICATION PROTOCOL**:
   Before including ANY fact in your response, verify:
   - Does this fact DIRECTLY relate to the primary subject?
   - Is this information SPECIFICALLY about what was asked?
   - Could this information be about a DIFFERENT entity mentioned in the same source?
   
3. **MIXED CONTENT HANDLING**:
   When a search result mentions multiple entities/products/topics:
   - DO NOT assume all information applies to the query subject
   - Explicitly identify which entity each piece of information refers to
   - Only use information that clearly relates to the PRIMARY SUBJECT
   - If ambiguous, prefer to OMIT rather than risk mixing information
   
4. **ENTITY DISTINCTION STRATEGIES**:
   Use these signals to distinguish between entities:
   - **Pricing differences**: Different price points usually indicate different products
   - **Feature sets**: Distinct features often signal different offerings
   - **Names and identifiers**: Even slight name variations may indicate different entities
   - **Temporal context**: Different release dates or availability periods
   - **Target audience**: Different user segments or use cases
   
5. **INFORMATION ATTRIBUTION RULES**:
   - Always specify WHICH entity/product information refers to
   - Use phrases like "Specifically for [entity]..." or "Regarding [entity]..."
   - When uncertain, say "Information found but unclear if it applies to [query subject]"
   - Never merge information from different entities into one response
   
6. **CONFLICT RESOLUTION FOR MIXED SOURCES**:
   When sources discuss multiple related entities:
   - Extract ONLY information with clear attribution to the query subject
   - Ignore or explicitly exclude information about other entities
   - If a source primarily discusses a different entity, note this and use cautiously
   - State clearly when filtering: "Focusing only on [query subject]..."
...
Answer using this automatic conflict resolution protocol while maintaining transparency about information sources and any corrections made."""
    ...
    if tool_context:
        ...
        # Check if this contains search results that need disambiguation warnings
        is_search_result = 'SEARCH RESULTS FROM' in tool_context or 'PRIMARY SUBJECT:' in tool_context
        
        # Enhanced label for search results with entity disambiguation
        if is_search_result:
            label = f'🔍 SEARCH RESULTS (PRIORITY 1 - MOST AUTHORITATIVE - {current_time.strftime("%H:%M UTC")})\n⚠️ ENTITY DISAMBIGUATION APPLIED: Information filtered to match query subject only'
        else:
            label = f'🔍 SEARCH RESULTS (PRIORITY 1 - MOST AUTHORITATIVE - {current_time.strftime("%H:%M UTC")})'
        ...
        info_sources.append({
            ...
            'metadata': {
                ...
                'entity_filtering_applied': is_search_result
            }
        })
        ...
        async for update in executor.execute_task_intelligently(
            ...
            if update.get("type") == "tool_result":
                ...
                if result and result.success:
                    ...
                    # Build search context with enhanced formatting
                    from app.core.search_result_formatter import format_search_results_for_llm
                    if 'search' in result.tool_name.lower() or 'google' in result.tool_name.lower():
                        formatted_result = format_search_results_for_llm(result.tool_name, result.result)
                        search_context += f"\n### {result.tool_name} Results:\n{formatted_result}\n"
                    else:
                ...
                try:
                    ...
                    if results and results[0].confidence >= threshold and results[0].suggested_tools:
                        ...
                        if direct_result and not direct_result.get("error"):
                            ...
                            # Use enhanced formatter for search results
                            from app.core.search_result_formatter import format_search_results_for_llm
                            if 'search' in tool_name.lower() or 'google' in tool_name.lower():
                                search_context = f"\n### {tool_name} Results:\n{format_search_results_for_llm(tool_name, direct_result)}\n"
                            else:
                ...
                if successful_tool_results and not skip_knowledge_search:
                    ...
                    for tr in successful_tool_results:
                        ...
                        if 'result' in tr and tr.get('success', False):
                            tool_name = tr['tool']
                            # Use enhanced formatter for search tools
                            if 'search' in tool_name.lower() or 'google' in tool_name.lower():
                                formatted_result = format_search_results_for_llm(tool_name, tr['result'])
                                tool_context += f"\n### {tool_name} Results:\n{formatted_result}\n"
                            else:
                                tool_context += f"\n{tool_name}: {json.dumps(tr['result'], indent=2)}\n"
                        elif 'error' in tr:
