#!/usr/bin/env python3
"""
Fix the prompt template by escaping JSON braces that conflict with Python format()
"""
...
def fix_prompt_template():
    """Fix the template by escaping JSON braces"""
    
    fixed_prompt = """You are an expert knowledge graph extraction system specializing in extracting complete, meaningful entities from business and technical documents. Extract ONLY high-quality, complete entities and relationships.
            ...
            if result:
                ...
                for prompt in settings['prompts']:
                    if prompt.get('prompt_type') == 'knowledge_extraction' or prompt.get('name') == 'Knowledge Extraction Prompt':
                        prompt['prompt_template'] = fixed_prompt
                        updated = True
                        logger.info("Updated existing knowledge_extraction prompt with fixed braces")
                        break
                ...
                if not updated:
                    settings['prompts'].append({
                        ...
                        "description": "Enhanced prompt for high-quality entity extraction (fixed braces)",
                        "version": 3,
                        "is_active": True
                    })
                    logger.info("Added new knowledge_extraction prompt with fixed braces")
                ...
                conn.commit()
                logger.info("✅ Successfully updated extraction prompt with fixed JSON braces")
